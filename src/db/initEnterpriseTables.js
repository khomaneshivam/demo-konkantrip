...
/**
 * Ensures employee_sessions and audit_trail tables exist
 */
async function initEnterpriseTables() {
    try {
        await db.query(`
        ...
        // Check if `employee_roles` has `object_permissions` column or ensure default roles are setup
    } catch (error) {
        console.error("Warning: Error initializing enterprise session/audit tables:", error.message);
    }
}

module.exports = { initEnterpriseTables };
