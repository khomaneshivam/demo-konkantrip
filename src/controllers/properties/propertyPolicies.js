    ...
    try {
        ...
        if (existing.length === 0) {
            ...
            if (updateFields.length > 0) {
                ...
                await db.query(`UPDATE property_policies SET ${setClauses} WHERE property_id = ?`, values);
            }
        }

        const [saved] = await db.query("SELECT * FROM property_policies WHERE property_id = ? LIMIT 1", [propertyId]);
